...
class Calculator:
    def __init__(self, master):
        ...
        for row in range(4):
            for col in range(4):
                button = tk.Button(master, text=buttons[row][col], width=5, height=2,
                                   font=('Arial', 14), command=lambda row=row, col=col: self.button_click(buttons[row][col]))
                button.grid(row=row+1, column=col, padx=5, pady=5)

        # Create a button to clear the entry widget
        clear_button = tk.Button(master, text='C', width=5, height=2, font=('Arial', 14), command=self.clear)
        ...
        delete_button = tk.Button(master, text='DEL', width=5, height=2, font=('Arial', 14), command=self.delete)
